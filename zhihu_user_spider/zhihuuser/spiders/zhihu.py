...
class ZhihuSpider(scrapy.Spider):
    ...
    start_user = 'a-ha-37-91'  # 开始放进去的第一个用户的ID
    ...
    def start_requests(self):
        ...
        # 上面是访问第一个用户，获取详细信息
        yield Request(url=self.followers_url.format(user_name=self.start_user, include_follow=self.include_follow, offset=0, limit=20),
                      callback=self.get_followers_parse)
        # 上面是访问第一个用户的粉丝列表，下面是访问关注列表
        yield Request(url=self.followees_url.format(user_name=self.start_user, include_follow=self.include_follow, offset=0, limit=20),
    ...
    def get_user_info(self, response):  # 获取用户信息信息
        ...
        yield item
        print(item["name"])
        yield Request(url=self.followers_url.format(user_name=data.get('url_token'), include_follow=self.include_follow, offset=0, limit=20),
                      callback=self.get_followers_parse)
        yield Request(url=self.followees_url.format(user_name=data.get('url_token'), include_follow=self.include_follow, offset=0, limit=20),
                      callback=self.get_followees_parse)
            ...
            try:
                if followers_data.get('data'):  # data里面是一个由字典组成的列表，每个字典是粉丝的相关信息
                    for one_user in followers_data.get('data'):
                        ...
                        yield Request(url=self.userinfo_url.format(user_name=user_name, include_userinfo=self.include_userinfo),
